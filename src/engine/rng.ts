 ...
 *
 * The host generates a seed once and broadcasts it in the round start; every
 * peer constructs an identical RNG from it, so decks, seat placement and the
 * per-turn resolution order are IDENTICAL on all clients without syncing a
 * single value. Never use Math.random() for anything peers must agree on.
 *
