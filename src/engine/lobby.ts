 ...
 * first round and starting a rematch are the same code path.
 */
...
export interface LobbyConfig {
  ...
  minPlayers?: number;
  maxPlayers?: number;
  /** Rendered under the roster so guests can see the HOST's chosen mode. */
  modeLine?: () => string;
  onCancel?: () => void;
}
...
/** Read ?room= once, without minting anything. Null when there is no deep link. */
...
export function createLobby(config: LobbyConfig): { destroy: () => void } {
  ...
  async function share(): Promise<void> {
    ...
    const shareData = { title: 'Join my game of Frostward', text: `Room ${config.roomCode}`, url: link };
    if (navigator.share) {
      try {
  ...
  function render(): void {
    ...
    const ps = players();
    const mode = config.modeLine?.() ?? '';
    const key = JSON.stringify([ps, s.canStart, s.voted, net.hostSettled(), mode]);
    ...
    container.innerHTML = `
      <div class="lobby">
        <div class="lobby-head">
          <h2 class="lobby-title">Room <span class="lobby-code selectable">${escapeHtml(config.roomCode)}</span></h2>
          <p class="lobby-sub">${ps.length}/${maxPlayers} players · peer-to-peer, no server</p>
        </div>
        <div class="lobby-invite">
          <input class="lobby-link selectable" readonly value="${escapeHtml(link)}" aria-label="Invite link" />
        ...
        ${mode ? `<p class="lobby-mode">${escapeHtml(mode)}</p>` : ''}
        ${
          !net.hostSettled()
