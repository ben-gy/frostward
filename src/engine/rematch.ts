 ...
 *     bytes. In Frostward the index IS the seat, so a disagreement here would
 *     have two peers defending each other's hearths.
 *
...
export interface RoundInfo<O = unknown> {
  ...
  isHost: boolean;
  /**
   * The host's game settings for this round — the mode, and anything else that
   * changes the shape of the board. Travels WITH the start for the same reason
   * the roster does: a setting each peer reads from its own UI is a setting two
   * peers can disagree about, and then they are playing different games.
   */
  opts: O;
}
...
export interface RoundsConfig {
   ...
   * the round for them before starting anyway. Default 8s.
   */
  graceMs?: number;
  /**
...
interface VoteMsg {
   ...
   * the wrong round and silently dropped: a soft-deadlock.
   */
  cur?: number;
  /**
   ...
   * to play, rather than each guest rendering its OWN setting as the host's.
   */
  opts?: unknown;
}
...
export function createRounds(config: RoundsConfig): Rounds {
  ...
  // as it arrives, so a lobby can render real names rather than "…". One
  // protocol covers presence, the first round and every rematch — there is no
  // second start path to drift out of sync.
  const sendVote = net.channel<VoteMsg>('rv', (msg, from) => {
    ...
    // than ours, we are the one that fell behind — adopt it. Only catch up while
  ...
  function maybeAutoStart(): void {
    ...
    // build deadlocked: one player still reading the summary held the whole room
    // hostage. Give the stragglers a visible countdown, then start without them.
