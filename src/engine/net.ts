 ...
 * FREE public infrastructure for the initial handshake — no server of our own,
 ...
 * `trystero/mqtt` if relays are flaky in your region.
 *
 * Netcode model this wrapper assumes: **host-authoritative star**. In Frostward
 * the host only adjudicates WHICH commits happened — the turn resolution itself
 * is a pure function of (seed, mode, roster, commits), so every peer recomputes
 * an identical board and nothing but the commits ever crosses the wire.
 *
 ...
 *  - Action names (channels) must be <= 12 bytes. Keep them short: 'cmt','trn'.
 ...
 * (strategy.js) while `room.leave()` is ASYNC and defers its real teardown behind
 * a ~99ms timer (room.js). So `net.leave(); createNet(...)` in the same tick hands
 * you back the very room object that is about to be destroyed. Moments later the
 * deferred teardown clears the announce timer and unsubscribes from every relay
 * — the "fresh" Net is a corpse: permanently deaf, roster of one, and every peer
 * elects itself host. Both players sit in the right room code, alone.
 *
...
export interface NetConfig {
  /** Namespaces the game on the shared signaling infra. Use the repo slug. */
  appId: string;
   ...
   * via a link, a typed code, or a deep link must leave this false, or two
...
export interface Net {
   ...
   * settled, so a peer whose mesh has not formed never acts as host.
   */
  isHost(): boolean;
  /**
   * False for the first moments in a room, while we wait to hear from an
   * incumbent host. Render "connecting…" rather than a host badge until true.
   */
  hostSettled(): boolean;
   ...
   * receivers and both fire. Use `send.off()` to detach one.
   */
  channel<T = NetData>(
    name: string,
   ...
   * Resolves once Trystero has actually retired the room.
   */
  leave(): Promise<void>;
}
...
export function createNet(config: NetConfig, handlers: NetHandlers = {}): Net {
  ...
  // during the seconds before the mesh forms EVERY peer painted itself host.
  //
  // So: the host ANNOUNCES and everyone else ADOPTS. A joiner is `unsettled`
  // until it hears an announce. The incumbent keeps the role for as long as it
  // is in the room. Only when it LEAVES do the survivors elect again (min-id).
  ...
  getHost((msg, from) => {
    ...
    // instant, or a partition just healed. Both sides apply the same rule.
    setHost(from < currentHost! ? from : currentHost!);
  });

  if (config.claimHost) {
    // This peer minted the code, so there is no incumbent to defer to.
    setHost(selfId);
  } else {
  ...
  room.onPeerJoin((id) => {
    ...
    // the newcomer who is in charge so it can settle without waiting a timer.
